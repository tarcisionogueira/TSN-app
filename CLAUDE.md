...
| `no-use-before-define` (dentro de `verificar:sintaxe`, só `src/`) | idem | Variável usada no MESMO escopo antes do `const` — em componente React é tela inteira quebrada ("Cannot access 'X' before initialization"). Nasceu em 24/09: `/buscar` ficou fora ~5 h. `variables: false` ignora uso dentro de callback (que roda depois e é seguro) |
