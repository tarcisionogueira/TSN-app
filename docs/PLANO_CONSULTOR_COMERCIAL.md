...
> revela+registra → feedback na trilha. Concluídas F2 (tela /comercial) e F3 (atribuição ?ref + Admin). Concluída F4 (Atendimento com escopo). **TODAS AS 6 FASES CONCLUÍDAS (21/08)** — plano implementado de ponta a ponta; ver as decisões em aberto (§9) que seguem com o dono (comissão, SLA definitivo, carteira).
...
| F5 ✅ | NPS: rota pública tokenizada + cron 15d + e-mail Resend — concluída 21/08 (invariantes de contradição → F6) | feita |
| F6 ✅ | Admin › Comercial (visão total + trilha + NPS + CSV) + invariantes qa — concluída 21/08 | feita |
