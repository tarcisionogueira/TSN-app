...
- **Memória por cliente (Handoff automático): ✅ JÁ EXISTE.** `perfis.memoria_ia` + `api/resumir-ticket.js` (resumo rolante a cada atendimento fechado) + `ChatSuporte.jsx` carrega na volta. Como é **por usuário** (não por canal), a continuidade **site → WhatsApp** vem de graça quando o número casar com o cadastro. No wiring do WhatsApp, além da memória, carregar também as mensagens do chamado aberto (continuidade no meio da conversa).
- **Botão "Falar por aqui / Falar no WhatsApp": ✅ PREPARADO.** A opção "Falar no WhatsApp" já está no chat (`ChatSuporte.jsx`), **oculta** enquanto `VITE_WHATSAPP_NUMERO` estiver vazia. Quando a integração gerar o número, setar `VITE_WHATSAPP_NUMERO` (só dígitos, ex.: `5571999999999`) na Vercel + redeploy → o botão aparece sozinho, sem mexer no código.
