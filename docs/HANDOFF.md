...
- **⏳ RETENÇÃO Etapa 2 — DESENHO PRONTO, aguardando 3 confirmações do dono** (é destrutivo → "notificar→esperar→apagar", nunca apaga sem aviso). Mecanismo: (1) botão **"Arrematei este imóvel"** → grava `arremate_sinalizado_em`; (2) **cron de avisos** acha imóveis entrando na janela e notifica ANTES (marca `notificado_em`) — Regra 2: 3-relatórios não sinalizado, avisa aos 10d e apaga aos 15d; Regra 1: arrematado que parou de pagar, avisa na hora e apaga 30d após; (3) `anexos_expirados` honra 1/2 só após notificado+prazo. **Decisões pendentes (com recomendação):** botão na Análise + Minhas Análises · e-mail+push, prazos 10/15d e 30d · "pagante" = assessorado adimplente (`inadimplenteDias`/Asaas). Construir ao receber o "segue".
...
- **⏳ PWA no menu + Landing (planejado):** item "Instalar app" no menu interno; nova aba **"Índice BidPro"** (consulta acima); na **tela inicial antes do login**, blocos de CTA "Baixe o app" + "Consulte o preço do m² na sua região" (aba pública = isca de topo de funil). Implementar ao aprovar.
