...
- **RETENÇÃO revisada (regras novas do dono) — em 5 etapas:** **(0)** Regra "sem os 3 relatórios + praça vencida → apaga 24h após a última praça" na RPC `anexos_expirados` (baixo risco, já reduz storage; venda-direta já é o comportamento atual). **(1)** *Pré-requisito:* estado **"arrematei este imóvel"** (novo botão/flag) + **notificações** (e-mail/push): "X dias p/ sinalizar", "pagamento parou → docs saem em 30 dias" — nada apaga ainda. **(2)** Regra "não arrematado + 3 relatórios → apaga 15 dias sem sinalização" (reunião deixa de ser gate, vira só registro). **(3)** Regra "arrematado → mantém enquanto paga; apaga 30 dias após parar + notifica" (liga ao Asaas/`inadimplenteDias`). **(4)** Registro de reuniões p/ relatório (quem/tipo/praça com-sem investidor). Etapas 2–3 são **destrutivas** → só depois da Etapa 1.
- **PWA — em 3 etapas:** **(1)** instalável (manifest + `vite-plugin-pwa` modo **injectManifest** p/ preservar o `public/sw.js` de push + precache do shell + ícones maskable + botão instalar + instruções iOS + **estratégia de atualização**) ~1–2 dias. **(2)** offline gracioso + fluxo de update testado ~2–3 dias. **(3)** push no iPhone (instalar destrava iOS 16.4+) + testes ~0,5–1 dia. Paralela: **code-splitting** do bundle de 2 MB (hoje monolítico, tudo `import` estático em `App.jsx`). **Push já está 100% em produção** (`src/utils/push.js`, `public/sw.js`, `api/push-*`). **Dúvida do dono respondida:** é UM só código/deploy — push p/ `main` atualiza navegador E PWA instalado juntos (sem app store); dados são sempre ao vivo; a "casca" (bundle) atualiza via SW na próxima abertura/reload; a config de update (Etapa 1) evita ficar preso em shell velho.
- **LEITURA de IA nos documentos capturados (preencher telas):** **7.439 docs / 2.988 imóveis** já no Storage. Estimativa (Haiku 4.5 US$1/US$5): **~US$150–250 passada única** (~US$0,05/imóvel novo). Opus 4.8: ~US$800–900. Só inferência — **Bright Data é à parte**, só p/ os **~3.676 sem avaliação** (aguarda OK de custo). Recomendação: Haiku, Opus só nos incertos.
- **BACKLOG TRT-15 (`docs/LEILOEIROS_TRT15_BACKLOG.md`):** 58 domínios; **16 já cobertos** via portal (custo ~0); **~41 pendentes** (2 a confirmar: `brunoleiloes`/`albertomacedo`; 39 sem match). Graus: **1-Baixo** (recon: já cai no LJUD/LEILOTECH/SUPORTE/SUPERBID?), **2-Médio** (plataforma white-label onboarda vários — candidatos: `e-leiloeiro`, `centraljudicial`, `leilaobrasil`, `hastapublica`, `judhastas`), **3-Alto** (scraper dedicado `scraper-puppeteer.mjs`). Grau real só após o **recon (passo 1)**, ainda não feito p/ os 39.
