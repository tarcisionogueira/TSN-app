...
## ✅ `VITE_TURNSTILE_SITE_KEY` — configurada em 16/09

Widget criado no Cloudflare (`BidPro Brasil - Cadastro`, domínio `bidprobrasil.com.br`, modo
Managed). As duas pontas foram configuradas pelo dono no mesmo dia:
1. `VITE_TURNSTILE_SITE_KEY` criada na Vercel (Production + Preview + Development).
2. Secret Key colada em Supabase → Authentication → **Attack Protection** (não é
   "Bot and Abuse Protection" nesta versão do painel — o nome mudou) → Enable Captcha
   protection → Turnstile by Cloudflare.
