-- Achado ao vivo do dono (21/09): contrato de locação comercial assinado por 3 partes
-- chegou cortado no MEIO de uma cláusula ("...ocultação de patrimônio ilícito; (" sem o
-- item seguinte). Causa raiz: api/gerar-contrato.js usava sanitizeText(conteudoDireto,
-- 20000) — pensada pra limitar texto CURTO indo pra prompt de IA, aplicada ao CONTRATO
-- INTEIRO. Corrigido no código (teto subiu pra 500000). Mas o corte era SILENCIOSO — dois
-- contratos de 04/08 foram ASSINADOS por clientes reais com o mesmo defeito, e ficaram
-- assim por mais de um mês sem ninguém notar. Este invariante é a rede de segurança:
-- `length(conteudo)` batendo EXATO num teto antigo/atual de sanitização é a assinatura do
-- corte (mesmo princípio de `length(descricao) = teto` já usado pras descrições de leilão)
-- — dificilmente um contrato real termina num número redondo desses por coincidência.
