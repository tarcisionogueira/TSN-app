-- 14/09: achado real do dono — imóvel Giordano Leilões (Guarulhos/SP, id
-- 546e44df-84ef-4296-bd32-78f04d78b05c) mostrava "IPTU em aberto: R$ 21.438.724,24" na aba
-- "Confirmado na documentação", 37x o valor de avaliação (R$ 572.434). Conferido pelo dono
-- direto no site do leiloeiro: a dívida não existe lá. Causa provável (ver comentário em
-- api/_doc-extracao.js, mesmo commit): leilão JUDICIAL cujo PDF reúne vários lotes do mesmo
-- processo — publicarFatosDoPdf() (scripts/captura-documentos.mjs) lê o documento inteiro sem
-- recortar por lote, e o teto de sanidade de extrairCustosTexto() para "débito de IPTU"
-- aceitava qualquer valor até R$ 50 MILHÕES (agora R$ 2 milhões — ver o outro arquivo).
--
-- 1) Corrige o dado já contaminado deste imóvel específico (o teto novo não retroage sozinho
--    a quem já foi salvo acima do limite antigo).
update public.imoveis_leilao
   set doc_fatos = doc_fatos #- '{custos,iptuDebito}'
 where id = '546e44df-84ef-4296-bd32-78f04d78b05c'
   and (doc_fatos->'custos'->>'iptuDebito')::numeric > 2000000;

-- 1b) O invariante abaixo (item 2) provou o padrão MAIS AMPLO do que o teto absoluto sozinho
--     cobre: dois imóveis do SODRE em Cotia (ids d5319efd-1381-46d4-87a3-89398f038dac e
--     73a56082-a654-47c9-affe-6e777c8c3189) tinham o MESMO condominioDebito = R$ 1.045.702,85
--     estampado nos dois — abaixo do teto de R$ 2 milhões, mas acima do próprio valor de
--     avaliação de cada um (R$ 258.857). Mesma assinatura do caso Giordano: valor do documento
--     (provavelmente dívida agregada do condomínio, ou de outra unidade do mesmo edital)
--     contaminando o registro individual do lote. Corrige os dois:
update public.imoveis_leilao
   set doc_fatos = doc_fatos #- '{custos,condominioDebito}'
 where id in ('d5319efd-1381-46d4-87a3-89398f038dac','73a56082-a654-47c9-affe-6e777c8c3189')
   and (doc_fatos->'custos'->>'condominioDebito')::numeric > coalesce(valor_avaliacao, valor_minimo_ref);

-- 2) Invariante novo em qa_invariantes(): débito de IPTU/condomínio extraído do documento
-- maior que o próprio valor do imóvel é o sintoma que expôs este caso e serve para pegar
-- qualquer outro já contaminado (teto antigo ainda pode ter deixado passar valores abaixo
-- de R$ 2 milhões mas ainda maiores que o imóvel) e recorrências futuras da mesma causa.
...
as $function$
  ...
  inv(chave, titulo, categoria, gravidade, valor, limite) as (
    values
     ...
     -- NOVO (14/09): débito de IPTU/condomínio extraído do documento maior que o valor do
     -- próprio imóvel — sintoma que expôs a contaminação entre lotes do mesmo PDF (ver
     -- comentário em api/_doc-extracao.js). Pega o que já está salvo hoje E qualquer
     -- recorrência futura, independente do teto absoluto usado no extrator.
