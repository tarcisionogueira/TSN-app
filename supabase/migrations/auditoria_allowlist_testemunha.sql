-- Adiciona get_contrato_testemunha à allowlist do auditor (allow_anon_definer), mesmo padrão
-- SEGURO do get_contrato_por_token: SECURITY DEFINER exposto a anon, casado por TOKEN exato e
-- devolvendo só campos públicos da testemunha (nunca KYC/CPF da parte). Sem isso o auditor
-- acusa "atenção" (rpc_definer_anon). Aplicada em produção via MCP; auditoria voltou a 0/0.
-- Corpo idêntico ao vigente + 1 entrada no array allow_anon_definer.
...
declare
  ...
  allow_anon_definer text[] := array[
    'get_contrato_por_token','get_contrato_testemunha','is_admin','is_equipe','app_role','email_existe',
