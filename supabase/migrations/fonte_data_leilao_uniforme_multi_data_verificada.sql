-- 20/09: `fonte_data_leilao_uniforme` alertava TORRES3 (179 lotes em 02/10 + 1 lote em 09/10,
-- 71 cidades, 13 UFs, 179 matrículas distintas — leilão único nacional, mesma assinatura já
-- verificada em HASTA em 25/08 e 01/09) porque a cláusula de isenção só cobria fonte com 1
-- ÚNICA data distinta (`u.nd = 1`), nunca 2 — mesmo a detecção aceitando `nd <= 2` (o padrão
-- normal de 1ª/2ª praça do mesmo leilão). Generaliza: isenta quando TODAS as datas distintas
-- da fonte (não só a menor) estão verificadas em `fonte_data_uniforme_verificada`.
insert into public.fonte_data_uniforme_verificada (fonte, data_leilao, verificado_em, evidencia) values
  ('TORRES3','2026-10-02', now(), '179 lotes ativos, 179 matriculas distintas, 150 valor_minimo distintos, 71 cidades, 13 UFs — parser lendo por lote (scraper-soleon.mjs), nao carimbando. Leilao unico nacional (3 Torres Leiloes), mesma assinatura ja verificada em HASTA (28/08 e 03/09).'),
  ('TORRES3','2026-10-09', now(), 'Lote isolado (Franca/SP, item 72576) com praca propria em data diferente do leilao nacional principal — 2a praca ou lote destacado do mesmo evento, nao carimbo de bug.')
