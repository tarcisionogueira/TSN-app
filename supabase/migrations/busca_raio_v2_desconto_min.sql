-- Adiciona o filtro de DESCONTO mínimo à busca por raio (RPC buscar_por_raio_v2).
-- O filtro de desconto existia só no caminho SEM raio; no modo raio (que passa pela
-- RPC) ele era ignorado. Aqui a RPC ganha o parâmetro desconto_min e o aplica.
DROP FUNCTION IF EXISTS public.buscar_por_raio_v2(double precision, double precision, double precision, integer, integer, text[], text, text[], text[], double precision, double precision);
...
AS $function$
  WITH filtrados AS (
    ...
    WHERE
      ...
      AND (i.valor_minimo IS NULL OR i.valor_minimo >= valor_min)
      AND (i.valor_minimo IS NULL OR i.valor_minimo <= valor_max)
