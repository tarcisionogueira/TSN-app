-- Filtro de forma de pagamento na busca por raio (RPC buscar_por_raio_v2) alinhado
-- ao front (aplicarFiltrosImoveis). Buckets por MODALIDADE, mutuamente exclusivos:
--   hipotecado = leilão JUDICIAL (arrematação parcelável, art. 895 CPC) OU
--                forma_pagamento='hipotecado'. O scraper marca todo judicial como
--                'a_vista' (o parcelamento só consta no edital), por isso a chave é
--                a MODALIDADE, não a forma_pagamento — antes exigia
--                forma_pagamento='financiado' AND judicial e dava 0 resultados.
--   financiado = NÃO-judicial com forma_pagamento='financiado'.
--   a_vista    = NÃO-judicial pago à vista ("não a vista" para os judiciais).
