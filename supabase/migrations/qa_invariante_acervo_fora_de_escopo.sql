-- 20/09: adiciona invariante `acervo_fora_de_escopo` ao qa_invariantes(), espelhando
-- `fora_do_acervo_imovel_veiculo()` (criada em acervo_so_imovel_veiculo.sql) — pega regressão
-- futura (nova fonte multi-bem sem o filtro de ingestão) sem depender de screenshot do dono.
...
AS $function$
  ...
  inv(chave, titulo, categoria, gravidade, valor, limite) as (
    values
     ...
     -- ESCOPO DELIBERADAMENTE RESTRITO às 4 fontes que passam por checarQualidade() em
     -- scraper-core.mjs (LEILAOBRASIL/LUTHERO/LEILAOPRO/EMILIOMATOS) — onde o classificador foi
     -- testado e validado contra dado real. Testado (NÃO aplicado) contra o acervo inteiro em
     -- 20/09: 365 "positivos" em 22 fontes que usam abreviação de título (SUPERBID "Apto 45m²",
     -- LJUD "Sala 701 - 200,44m²", "Box nº 02"...) que este classificador NUNCA viu — quase toda
     -- a massa é FALSO positivo (imóvel real com título terso), não contaminação de verdade.
     -- Alargar pra essas fontes exige validar o classificador contra o estilo de título de CADA
     -- uma antes — a mesma lição de sempre: medir contra dado real antes de aplicar.
     ('acervo_fora_de_escopo','Lote ativo (nas fontes já filtradas na ingestão) que nao e nem imovel nem veiculo','Captura','bug',
       (select count(*) from imoveis_leilao where ativo
          and fonte in ('LEILAOBRASIL','LUTHERO','LEILAOPRO','EMILIOMATOS')
          and public.fora_do_acervo_imovel_veiculo(titulo, descricao)), 0),
