-- Bug bounty 31/07 — APLICADO. O auditor auditoria_seguranca() NÃO cobria VIEWS
-- SECURITY DEFINER com PII exposta a anon/authenticated — foi a lacuna que deixou
-- public.saldo_usuarios (nome+chave_pix+saldo de todos) vazar para anon sem o auditor
-- acusar. Adiciona o check view_definer_pii (crítico): view em public sem
-- security_invoker, legível por anon/authenticated, com coluna chave_pix/cpf/cpf_enc.
-- (Definição completa da função no banco; este arquivo documenta a mudança — o corpo
-- é idêntico ao vigente + o novo bloco. Ver pg_get_functiondef para o texto integral.)
...
begin
  ...
  where n.nspname='public' and p.prosecdef and p.prorettype <> 'pg_catalog.trigger'::regtype
    and has_function_privilege('anon', p.oid, 'EXECUTE') and not (p.proname = any(allow_anon_definer));
  ...
  return jsonb_build_object('gerado_em', now(), 'total', jsonb_array_length(achados),
