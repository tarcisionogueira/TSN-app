-- Achado 22/09 (pedido do dono, investigando lote de areia aparecendo no site do leiloeiro
-- ao clicar "Acessar leiloeiro"): o invariante `acervo_fora_de_escopo` (que aplica
-- `fora_do_acervo_imovel_veiculo()` contra o acervo ativo) tem uma allowlist de 14 fontes,
-- e EDITAL_DJEN (437 ativos) e VLANCE (71 ativos) ficaram de fora — justamente as duas
-- fontes mais expostas a bem de tipo variado (DJEN é IA lendo texto de edital judicial
-- qualquer; VLANCE é agregador multi-leiloeiro white-label). Hoje (22/09) a checagem contra
-- o acervo INTEIRO, sem esse recorte, deu 0 ocorrências nas duas — não havia achado real no
-- momento —, mas o ponto cego de monitoramento contínuo é real e some com este fix.
--
-- Nota: a regex de `fora_do_acervo_imovel_veiculo()` pode ter falso-negativo em EDITAL_DJEN
-- por causa do boilerplate jurídico padrão ("em caso de imóveis e veículos, o pagamento
-- poderá ser parcelado...") que aparece em quase todo edital de leilão judicial, imóvel ou
-- não — isso não é resolvido aqui, é limitação conhecida da heurística por palavra-chave.
...
as $function$
  ...
  inv(chave, titulo, categoria, gravidade, valor, limite) as (
    values
     ...
     -- 22/09: EDITAL_DJEN e VLANCE entraram na allowlist (eram as duas fontes mais expostas
     -- a bem fora de escopo — IA lendo edital judicial qualquer, e agregador multi-leiloeiro
     -- — e ficavam sem monitoramento nenhum). Ver comentário no topo da migração.
