-- Cliente 360: expõe (para AUDITORIA sob demanda) os imóveis encaminhados por
-- e-mail/push (alertas_enviados) e o aprendizado de interesse × desinteresse do
-- cliente (feedback_imovel: 'interesse' / 'sem_interesse'), cada um com cidade/UF/
-- desconto do imóvel — para conferir se a seleção bate com a região e os filtros.
-- SECURITY DEFINER + search_path '' (bypassa a leitura só-admin, como o restante da RPC).
...
AS $function$
  select jsonb_build_object(
    ...
    'emails_total', (select count(*) from public.emails_log el
        where el.user_id = uid
           or lower(el.destinatario) = lower((select email from auth.users where id = uid))),
    -- NOVO: imóveis encaminhados por e-mail/push (auditoria da seleção por região/filtros).
    ...
    -- NOVO: aprendizado de interesse (widget "Sugestão para você" / feedback do cliente).
