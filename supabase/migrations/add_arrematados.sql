...
-- Documentos do imóvel/processo (visíveis ao arrematante)
...
-- Documentos pessoais do usuário (só o próprio usuário + admin/analista)
CREATE TABLE IF NOT EXISTS public.usuario_docs (
  ...
  tamanho_kb integer,
  criado_em timestamptz DEFAULT now(),
  expira_em timestamptz -- set to cancelamento + 90 days when subscription cancelled
);
