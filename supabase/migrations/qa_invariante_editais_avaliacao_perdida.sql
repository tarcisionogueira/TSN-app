...
-- suspeito"): 39 lotes EDITAL_DJEN ativos com valor_avaliacao NULL mesmo o edital de
-- origem (editais_leilao) trazendo o valor correto. Backfill aplicado direto (mesma
-- sessão) + achada e corrigida uma causa parcial (sanitiza_imovel_valores tratava
-- avaliação >= R$100mi como sentinela por FAIXA, zerando fazenda/imóvel rural genuíno —
-- ver migração sanitiza_imovel_valores_sentinela_exata_nao_faixa.sql). A causa completa
-- do gap ORIGINAL na promoção não foi 100% isolada nesta sessão — este invariante é a
-- rede de segurança pra não deixar o mesmo silêncio voltar sem ninguém notar, mesmo sem
-- o mecanismo exato confirmado.
