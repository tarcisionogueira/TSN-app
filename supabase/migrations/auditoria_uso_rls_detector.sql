-- Detector proativo de gaps de RLS que quebram o USO do usuário (mesma classe do
-- bug casos/"tenho interesse"): tabela com RLS + coluna de dono + authenticated pode
-- INSERT, porém SEM caminho de escrita do usuário (só admin, ou nenhuma política).
-- Exposto na auditoria de saúde (api/health-check.js) para pegar regressões ANTES
-- de o usuário topar com "new row violates row-level security policy".
--
-- Heurística: uma tabela está "coberta" se tem política INSERT/ALL citando a coluna
-- de dono OU o literal auth.uid() (políticas de usuário usam auth.uid(); as de admin
-- usam is_admin()/app_role(), que NÃO contam como escrita do usuário).
-- allowlist = tabelas escritas exclusivamente pelo servidor/API (service key) ou staff.
...
$$;
revoke all on function public.auditoria_uso() from public, anon, authenticated;
grant execute on function public.auditoria_uso() to service_role;
