-- 22/09: consequência direta do fix de desativar_leiloes_encerrados()/trg_desativa_leilao_
-- encerrado (retenção de 15 dias pro lote sem_lance/indeterminado ficar ATIVO — sem isso o
-- filtro "Sem lance" da Busca nunca mostrava nada, porque a desativação por hora sempre
-- corria na frente). Sem esta mesma exceção aqui, `leilao_vencido_ativo` passa a contar os
-- ~65 lotes intencionalmente retidos como se fossem bug — a MESMA classe de erro que este
-- projeto já nomeou (medir uma coisa, reportar com o nome de outra).
...
as $function$
  ...
  inv(chave, titulo, categoria, gravidade, valor, limite) as (
    values
       ...
       (select count(*) from imoveis_leilao
         where ativo and titulo ~ 'Lance Inicial:\s*R\$\s*[\d.]+,\d{2}'
           and abs(coalesce(valor_minimo,0) - replace(regexp_replace(
                 ...
                 '[^0-9,]', '', 'g'), ',', '.')::numeric) > 1), 0),
     ('leilao_vencido_ativo','Lote ativo com todas as praças já vencidas (achado 22/09: exclui sem_lance/indeterminado retido 15 dias — retenção intencional pra alimentar o filtro "Sem lance" da Busca, ver desativar_leiloes_encerrados)','Captura','bug',
       (select count(*) from imoveis_leilao i where i.ativo
          and public.leilao_ja_encerrado(i.data_leilao, i.data_leilao_2, i.data_fim, i.modalidade)
          and not (i.resultado_leilao in ('sem_lance','indeterminado') and i.resultado_apurado_em > now() - interval '15 days')), 0),
     ...
     ('acervo_fora_de_escopo','Lote ativo (nas fontes já filtradas na ingestão) que nao e nem imovel nem veiculo — baseline de titulo ambiguo em SUPERBID/LJUD tolerada, so o crescimento alem da folga e achado','Captura','gap',
       (select count(*) from imoveis_leilao where ativo
          and fonte in ('LEILAOBRASIL','LUTHERO','LEILAOPRO','EMILIOMATOS','SUPERBID','LJUD','VIP','BAYIT','LEILOTECH','KRONLEILOES','SBID9','GRUPOLANCE','FRAZAO','ALBERTOMACEDOLEILOES','EDITAL_DJEN','VLANCE')
          and public.fora_do_acervo_imovel_veiculo(titulo, descricao)), 15),
     ...
     ('doc_debito_maior_que_avaliacao','Débito de IPTU/condomínio extraído do documento maior que o valor do próprio imóvel (contaminação entre lotes do mesmo PDF)','Documentos','bug',
       (select count(*) from imoveis_leilao where ativo
          and coalesce(valor_avaliacao, valor_minimo_ref, 0) > 0
          and (
            coalesce((doc_fatos->'custos'->>'iptuDebito')::numeric, 0) > coalesce(valor_avaliacao, valor_minimo_ref)
            or coalesce((doc_fatos->'custos'->>'condominioDebito')::numeric, 0) > coalesce(valor_avaliacao, valor_minimo_ref)
          )), 0),
     ('contrato_texto_truncado','Contrato com texto cortado num teto de sanitização (achado de 21/09 — sanitizeText limitava o contrato inteiro, não só texto curto de prompt)','Documentos','critico',
       (select count(*) from contratos_link where conteudo is not null and length(conteudo) in (20000, 500000)), 0),
     ...
     ('fonte_data_leilao_uniforme','Fonte com 100+ lotes ativos e 1-2 datas de leilao (data em massa errada, ex.: PESTANA 26/10)','Captura','bug',
       (select count(*) from (
          select fonte, array_agg(distinct data_leilao::date) as datas
            from imoveis_leilao
         ...
         where exists (
           select 1 from unnest(u.datas) d(data_leilao)
            where not exists (select 1 from public.fonte_data_uniforme_verificada v
                                where v.fonte = u.fonte and v.data_leilao = d.data_leilao)
         )
        ), 0),
     ...
     ('mp_liberacao_atrasada','Pagamento MP aprovado com liberacao de dinheiro atrasada (>1 dia) vs. padrao observado de 0h (achado 21/09)','Financeiro','critico',
       (select count(*) from mp_pagamentos where status='approved' and dados_mp ? 'money_release_date' and dados_mp ? 'date_approved' and criado_em > now() - interval '30 days' and ((dados_mp->>'money_release_date')::timestamptz - (dados_mp->>'date_approved')::timestamptz) > interval '1 day'), 0),
     ...
     ('editais_avaliacao_perdida','Lote EDITAL_DJEN ativo sem valor_avaliacao mesmo o edital de origem tendo (achado 22/09: 39 casos — backfill aplicado, causa completa da promoção nao 100% isolada)','Captura','bug',
       (select count(*) from imoveis_leilao i join editais_leilao e on e.imovel_id = i.id
          where i.fonte='EDITAL_DJEN' and i.ativo
            and i.valor_avaliacao is null and e.valor_avaliacao is not null and e.valor_avaliacao > 0), 0)
  )
