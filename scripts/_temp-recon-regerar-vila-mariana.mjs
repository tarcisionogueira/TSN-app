/**
 * Descartável — regenera UM relatório mercadológico específico (Vila Mariana/SP,
 * imovelId 104c6b3a-4354-4f25-bfee-d17fdba6f92b) via o caminho CRON do
 * /api/gerar-analise (x-cron-secret + paraUserId), reaproveitando EXATAMENTE os
 * mesmos inputs (titulo/cidade/estado/imovel/mercadoInputs/parecerInputs) da
 * análise original (id 58058730-b5be-4f74-989f-24790e6aba23), para validar em
 * produção o fix de endereço ausente no parecer (commit 3af68e0). Grava direto
 * na conta do próprio dono (paraUserId = user_id da análise original) — não há
 * "espelhamento" a fazer porque já é a conta dele.
 */
...
const PARA_USER_ID = '92c713f3-1f1a-4758-bab2-32e6c83da433'; // Tarcisio (dono da análise original)
...
(async () => {
  console.log(`\n🔁 Regerando ${IMOVEL_ID} → ${BASE}/api/gerar-analise (paraUserId=${PARA_USER_ID})\n`);
  const t0 = Date.now();
  ...
  const parecer = j?.result?.parecer || j?.parecer || '';
  console.log(`✓ HTTP ${resp.status} (${secs}s) status=${j?.status || '?'}`);
  console.log(`  cita "Domingos de Morais"? ${parecer.includes('Domingos de Morais')}`);
  console.log(`  cita "R2V" (condomínio)? ${parecer.includes('R2V')}`);
  console.log(`\n--- abertura do parecer ---\n${parecer.slice(0, 500)}\n`);
})();
