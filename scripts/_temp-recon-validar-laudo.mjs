/**
 * Descartável — valida a leitura do laudo de avaliação (commit 5626cd4) regenerando de novo
 * o mesmo relatório da Vila Mariana (imovelId 104c6b3a-4354-4f25-bfee-d17fdba6f92b). Espera:
 * lerLaudoAvaliacao() encontrar o anexo tipo "laudo", ler o PDF real, e — se o valor divergir
 * do card (R$ 680.000) — corrigir imoveis_leilao.valor_avaliacao e citar no parecer.
 */
...
(async () => {
  console.log(`\n🔁 Regerando (com leitura de laudo) ${IMOVEL_ID} → ${BASE}/api/gerar-analise\n`);
  const t0 = Date.now();
  ...
  console.log(`✓ HTTP ${resp.status} (${secs}s)`);
  console.log(`  cita "laudo"? ${/laudo/i.test(parecer)}`);
  console.log(`  cita "débito" ou "ônus"? ${/débito|ônus/i.test(parecer)}`);
  console.log(`\n--- primeiros 1500 caracteres do parecer ---\n${parecer.slice(0, 1500)}\n`);
})();
