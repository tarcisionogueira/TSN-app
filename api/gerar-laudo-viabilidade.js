...
Retorne APENAS este JSON (sem markdown):
{
  ...
  "parecer": "Parecer de DEFESA (favorável ou contrária) da arrematação, em português formal, texto simples, estruturado com '§ SEÇÃO:' (sem markdown/asteriscos, sem travessão). Este relatório é a CONCLUSÃO: comece RESUMINDO os principais pontos de CADA relatório e só então construa a defesa. Use EXATAMENTE estas seções: § SEÇÃO: PRINCIPAIS PONTOS DO RELATÓRIO MERCADOLÓGICO (resuma em 3 a 5 tópicos curtos o que o relatório 1 concluiu: valor de mercado x aquisição, desconto real, potencial de renda/revenda, retorno estimado — só o que decide); § SEÇÃO: PRINCIPAIS PONTOS DO RELATÓRIO DOCUMENTAL E JURÍDICO (resuma em 3 a 5 tópicos curtos o que o relatório 2 concluiu: nível de risco, ônus/gravames relevantes, ocupação, débitos que o arrematante assume, diligências pendentes e a recomendação do jurídico); § SEÇÃO: CONTROLE DE QUALIDADE (confiança de cada relatório em 0-100 e contradições encontradas entre eles); § SEÇÃO: DEFESA DA DECISÃO (a tese central FAVORÁVEL ou CONTRÁRIA à arrematação, cruzando as duas frentes: por que os pontos fortes superam ou não os riscos; se favorável, defenda com números; se contrária, seja direto no motivo que inviabiliza); § SEÇÃO: CONDIÇÕES E DILIGÊNCIAS (o que precisa ser resolvido/confirmado antes do lance e o teto de lance sugerido, quando aplicável); § SEÇÃO: RECOMENDAÇÃO FINAL AO CLIENTE (o veredito em 1 a 2 frases, em linguagem de leigo: arrematar ou não, e a razão principal).",
  "resumoOperacao": ["FECHO DO RELATÓRIO, a última coisa que a pessoa lê antes de bater o martelo: 4 a 6 TÓPICOS curtos com o retrato do NEGÓCIO em linguagem de leigo, cruzando os dois relatórios. Cada tópico traz o número concreto. Cubra, quando houver o dado: (1) o que se arremata e a tese em 1 frase; (2) valor de mercado x valor de aquisição (lance mínimo) e o DESCONTO real, em % e em reais; (3) custos que o arrematante ASSUME (débitos, condomínio e, se ocupado, o custo e o prazo da desocupação); (4) INVESTIMENTO TOTAL estimado (lance somado aos custos); (5) RETORNO esperado (revenda em % e/ou locação com o aluguel e o yield mensal); (6) o TETO DE LANCE sugerido para a operação seguir valendo a pena. Não repita o parecer inteiro: é o resumo executivo do negócio, direto ao ponto."]
}`;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = {
      ...
      resumoOperacao: Array.isArray(parsed.resumoOperacao) ? parsed.resumoOperacao.filter(Boolean) : [],
