 ...
 *
 * ─── PROXY ISP (22/09) — em `_brightdata-isp-node.js`, NÃO neste arquivo ──────────────────
 * `buscarViaProxyIsp` (mesmo pedido do dono: quando a sub-cota diária do Web Unlocker recusar,
 * ex. apurar-resultado-leilao-cron.js/veículos SUPERBID, tenta o proxy ISP fixo por IP+tráfego
 * antes de desistir) precisa do pacote `undici` (`ProxyAgent`), que importa módulos nativos do
 * Node (`node:tls`, `node:net`, ...). Este arquivo (`_brightdata.js`) é importado também por
 * Edge Functions (ex. `api/baixar-doc.js`), e o Edge Runtime NÃO suporta esses módulos — nem
 * um `import('undici')` dinâmico escapa disso, porque o bundler da Vercel detecta o especificador
 * literal e tenta incluí-lo mesmo assim (quebrou o deploy inteiro em 22/09,
 * `NOW_SANDBOX_WORKER_EDGE_FUNCTION_UNSUPPORTED_MODULES`). Por isso `buscarViaProxyIsp` mora em
 * módulo PRÓPRIO (`_brightdata-isp-node.js`), importado só pelos chamadores Node.js
 * (`enriquecer-lote.js`, `apurar-resultado-leilao-cron.js`) — nunca por este arquivo.
 */
