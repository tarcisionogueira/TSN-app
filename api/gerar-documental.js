...
- "valorAvaliacaoOficial": o VALOR DE AVALIAÇÃO OFICIAL do imóvel (18/09, pedido do dono: a IA lê o laudo de avaliação junto com o resto da documentação, o cliente não precisa anexar nada à parte) — é o valor que o AUTO/LAUDO DE AVALIAÇÃO judicial atribuiu ao bem, ou o "valor de avaliação" que o edital/CEF publica separado do lance mínimo/valor de venda. NÃO confunda com o valor mínimo de venda/lance inicial (esse é o preço pedido no leilão, quase sempre COM deságio sobre a avaliação) nem com o valor que o arrematante pagou. Extraia só quando o documento nomear explicitamente "avaliação"/"valor de avaliação"/"laudo de avaliação"; se não constar, deixe 0 (não invente nem copie o valor mínimo de venda para este campo).
...
- FINANCIAMENTO A ASSUMIR = RESSALVA MÁXIMA, NAS DUAS MODALIDADES (pedido do dono, 13/08).
  ...
  existe uma DÍVIDA QUE VAI JUNTO COM O IMÓVEL, e o "desconto" aparente do lance pode ser menor
  que o saldo assumido. É a distância entre comprar um imóvel e comprar a posição de devedor.
  Quando identificar isso: severidade "bloqueante"; abra a seção 6 do parecer por este ponto,
  ANTES de qualquer outro; declare o SALDO em reais e o CREDOR se constarem, e se NÃO constarem
  diga com todas as letras que o valor da dívida assumida é desconhecido e precisa ser obtido
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // é uma questão muito preocupante. Sei que no laudo isso é apresentado, mas é interessante
    // colocar como uma ressalva maior."
    //
    // Por que é grave: no leilão judicial a REGRA é a arrematação extinguir os ônus e o credor se
    // satisfazer no preço (art. 908 §1º CPC / art. 1.499 CC) — quem arremata recebe livre. Quando
    // o edital inverte isso, existe uma dívida que ACOMPANHA o imóvel, e o desconto aparente do
    // lance pode ser menor que o saldo assumido. É a diferença entre comprar um imóvel e comprar
    // a posição de devedor.
    //
    // Por que aqui e não só no prompt: o modelo lê o campo e classifica, mas uma classificação
    // que depende de o modelo LEMBRAR não é garantia. Este risco é montado pelo servidor a
    // partir do campo extraído, entra na MESMA lista que a tela ordena por severidade, e por
    // isso não depende da redação do parecer para aparecer em destaque.
    const finAssumido = String(ex.financiamentoAssumido || '').toLowerCase() === 'sim';
    if (finAssumido) {
      const saldo = Number(ex.financiamentoSaldo) || 0;
      const credor = String(ex.financiamentoCredor || '').trim();
      const valorTxt = saldo > 0
        ? `O saldo indicado nos documentos é de R$ ${saldo.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`
        : 'O valor do saldo NÃO consta nos documentos lidos, e sem ele não é possível saber o custo real da arrematação';
      riscosAntifraude.push({
        categoria: 'Financiamento a assumir',
        severidade: 'bloqueante',
        constaNaDoc: true,
        descricao: `${ehJudicial ? 'Este leilão é JUDICIAL e, mesmo assim, ' : 'Neste leilão '}`
          + 'os documentos indicam que o arrematante ASSUME um financiamento já existente sobre o imóvel'
          + `${credor ? ` (credor: ${credor})` : ''}. Isso é o CONTRÁRIO da regra do leilão judicial, em que a `
          + 'arrematação extingue os ônus e o credor recebe do valor pago (o imóvel sai livre). Aqui a dívida '
          + `ACOMPANHA o imóvel: ela se soma ao lance, ao leiloeiro e às custas. ${valorTxt}. `
          + 'Some o saldo ao lance ANTES de decidir: o desconto real pode ser bem menor do que parece, ou '
          + 'inexistir. Confirme o valor atualizado diretamente com o agente financeiro antes de dar lance.',
      });
    }
    ...
    // VALOR DE AVALIAÇÃO OFICIAL (18/09, pedido do dono): a IA já leu o laudo de avaliação
    // junto com o resto da documentação — grava o número em imoveis_leilao.valor_avaliacao
    // para alimentar o card "Avaliação" (Arrematados.jsx), que hoje fica "—" em todo arremate
    // atribuído manualmente. SÓ PREENCHE LACUNA: nunca sobrescreve um valor já existente (ex.:
    // o que o scraper da Caixa já traz no feed) — o mesmo cuidado da ficha do imóvel acima.
    try {
