...
export async function leilaoEncerrado(sb, imovelId, dataDoCliente = null) {
  ...
  try {
    const [row] = await (await sb(
      `imoveis_leilao?id=eq.${encodeURIComponent(String(imovelId))}&select=data_leilao,data_leilao_2,ativo&limit=1`
    )).json();
    if (row) {
      candidatas.push(ms(row.data_leilao), ms(row.data_leilao_2));
