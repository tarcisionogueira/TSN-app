...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (reusoValido) {
      ...
      // ORÇAMENTO DA 1ª PASSADA — mantido como estava, e o motivo fica registrado (09/09).
      // Cheguei a ampliá-lo de 118s para 180s com base no cronômetro (as duas passadas morriam
      // exatamente no próprio teto). A tentativa seguinte MEDIU e refutou: com orcamentoA
      // 180,3s o Claude abortou no teto igual, e a 2ª passada nem coube (segundaCabe: false).
      // A causa não era o tamanho do orçamento — era o motor primário fora do ar
      // (Gemini: HTTP 403 "Your project has been denied access"), o que joga TODA busca no
      // fallback Claude web_search, que este arquivo já documenta como o caminho que aborta.
      // Ampliar o teto só trocava o instante da morte e ainda espremia a Etapa B: revertido.
