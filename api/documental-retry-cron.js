...
export default async function handler(req, res) {
  ...
  const q = `analises_documental?status=eq.concluida&result->>preliminar=eq.true&updated_at=gt.${encodeURIComponent(desde)}&order=updated_at.asc&limit=${LOTE}&select=user_id,imovel_id,titulo,cidade,estado`;
  let rows = [];
